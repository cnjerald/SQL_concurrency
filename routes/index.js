...
function getUpdateStatus2(pool) {
    return new Promise((resolve, reject) => {
        pool.getConnection((err, connection) => {
            if (err) {
            ...
            for (const entry of mergedArray) {
                ...
                try {
                    ...
                    try {
                        ...
                        setTimeout(() => {
                            console.log("Committing... after 5 seconds")
                        }, 5000); 
...
async function startRecoveryLoop2() {
    while (true) {
        console.log("Recovering logs!");
        try {
            const results = await Promise.all([
                getUpdateStatus2(node1Pool),
                getUpdateStatus2(node2Pool),
                getUpdateStatus2(node3Pool)
            ]);
            const allResults = results.flat();
